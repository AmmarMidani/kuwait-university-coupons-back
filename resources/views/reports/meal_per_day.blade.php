...
@section('pagename')
{{ __('website.meal_per_day_report') }}
@endsection
...
<div
    ...
    <div class="d-flex flex-column justify-content-center">
        <h4 class="mb-1">{{ __('website.meal_per_day_report') }}</h4>
        <p class="mb-0">{{ __('website.meal_per_day_report_slug') }}</p>
    </div>
    ...
    <h5 class="card-header">
        <div class="row">
            ...
            <div class="col-md-6 col-12 d-flex align-items-end justify-content-end">
                <button id="apply-filter-btn" type="button" class="btn btn-sm btn-outline-secondary waves-effect">{{
                    __('website.apply_filter') }}</button>
            </div>
        </div>
    </h5>
    <div class="card-body">
...
<script>
    ...
    $('#apply-filter-btn').on('click', function () {
        ...
        $.ajax({
            url: '{{ route("report.meal_per_day_generate_excel") }}',
            method: 'POST',
            data: {
                '_token': '{{ csrf_token() }}',
                date_from: date_from,
